...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      <Link
        href="/dich-vu"
        className="mt-4 inline-flex items-center gap-1.5 text-base font-medium text-ink-soft hover:text-crimson-600"
      >
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <span className="rounded-sm bg-gold-500 px-3 py-1 text-sm font-semibold text-wood-700">
            {service.levelLabel}
          </span>
          <h1 className="mt-4 text-balance font-display text-3xl leading-tight text-crimson-600 sm:text-4xl">
            {service.name}
          </h1>
          ...
          <div className="mt-8 aspect-[16/9] overflow-hidden frame-gold">
            <BrandVisual
              variant="crimson"
              src={service.image ?? "/images/spaces/tang-2-treatment.jpg"}
              label={service.name}
            />
          </div>
          ...
          <Block
            ...
            icon={<AlertTriangle className="h-5 w-5 text-crimson-600" />}
          >
            <ul className="grid gap-2 sm:grid-cols-2">
          ...
          <Block title="Cảm giác sau buổi trị liệu">
            <p className="border-l-2 border-gold-500 bg-cream p-8 font-display text-2xl italic leading-snug text-ink">
              “{service.feeling}”
            </p>
          </Block>

          <p className="mt-8 rounded-md border border-sand bg-cream p-8 text-base leading-relaxed text-ink-soft">
            Lưu ý: Liệu trình hỗ trợ thư giãn, lưu thông khí huyết và cải thiện cảm giác căng mỏi,
            góp phần cân bằng thân · tâm · trí. Không thay thế tư vấn y khoa chuyên môn.
          </p>
        </div>
            ...
            <dl className="mt-4 space-y-3 text-base">
              <div className="flex items-center justify-between">
                <dt className="flex items-center gap-1.5 text-ink-soft">
                  <Clock className="h-4 w-4" /> Thời lượng
                </dt>
