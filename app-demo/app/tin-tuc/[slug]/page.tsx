...
export default async function PostDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <Link
        href="/tin-tuc"
        className="inline-flex items-center gap-1.5 text-base font-medium text-ink-soft hover:text-crimson-600"
      >
        <ArrowLeft className="h-4 w-4" /> Tất cả bài viết
      </Link>

      <span className="mt-6 block text-sm font-semibold uppercase tracking-[0.15em] text-crimson-600">
        {post.cat}
      </span>
      <h1 className="mt-2 text-balance text-3xl leading-tight text-crimson-600 sm:text-4xl">
        {post.title}
      </h1>
      ...
      <article className="mt-8 space-y-5">
        ...
        {post.body.map((para, i) => (
          <p key={i} className="text-lg leading-[1.85] text-ink">
            {para}
          </p>
        ))}
      </article>
      ...
      <p className="mt-8 rounded-md border border-sand bg-cream p-6 text-base leading-relaxed text-ink-soft">
        Lưu ý: Nội dung mang tính chia sẻ kiến thức dưỡng sinh, không thay thế tư vấn y khoa chuyên
        môn.
      </p>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {related.map((p) => (
              <Link
                key={p.slug}
                href={`/tin-tuc/${p.slug}`}
                className="lift group flex overflow-hidden rounded-md border border-sand bg-cream"
              >
                <div className="relative w-28 shrink-0 overflow-hidden">
                  <Img src={p.image} alt={p.title} fill sizes="112px" className="object-cover" />
                </div>
                <div className="flex-1 p-5">
                  <span className="text-xs font-semibold uppercase tracking-[0.15em] text-gold-700">
                    {p.cat}
                  </span>
                  <h3 className="mt-1 text-lg leading-snug text-crimson-600 group-hover:text-crimson-700">
                    {p.title}
                  </h3>
