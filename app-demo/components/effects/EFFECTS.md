...
2. **Chỉ GSAP làm scroll.** ScrollTrigger/pin/scrub/marquee đều qua GSAP. Lenis đã nối vào
   `gsap.ticker` trong `components/motion/SmoothScrollProvider.tsx` — **KHÔNG** tạo
   `requestAnimationFrame` loop riêng.
...
6. **Tailwind 4 + token brand.** Component 3 bộ thường viết cho Tailwind 3 → sửa syntax
   ...
   (`--color-crimson-*`, `--color-gold-*`, … trong `app/globals.css`).
7. **Tái dùng preset motion.** Easing/thời lượng lấy từ `@/lib/motion/config.ts` và
   `@/lib/motion/easings.ts` (`registerEases`, `gsapEasings`, `motionConfig`, `breathFlow`) —
   không tự định nghĩa easing rời rạc.
